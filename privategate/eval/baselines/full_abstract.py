"""Baseline #3 — abstract every detected span, then send to the standard backend.

This stands in for "full-query paraphrase" sanitization. Every detected
span is replaced with its abstract superclass; everything else passes
through untouched.
"""
from __future__ import annotations

from privategate.backends.base import Backend
from privategate.backends.mock_standard import MockStandardBackend
from privategate.detector.rule_detector import RuleDetector
from privategate.eval.baselines.base import Baseline, BaselineRun, _timed
from privategate.eval.example import EvalExample
from privategate.rewriter.actions import apply_abstract


class FullAbstractBaseline(Baseline):
    name = "full_abstract"

    def __init__(self, backend: Backend | None = None) -> None:
        self._backend = backend or MockStandardBackend()
        self._detector = RuleDetector()

    def _abstract_all(self, text: str) -> str:
        spans = sorted(self._detector.detect(text), key=lambda s: s.start)
        out: list[str] = []
        cursor = 0
        for s in spans:
            if s.start < cursor:
                continue
            out.append(text[cursor:s.start])
            out.append(apply_abstract(s))
            cursor = s.end
        out.append(text[cursor:])
        return "".join(out)

    def run_one(self, example: EvalExample) -> BaselineRun:
        abstracted = self._abstract_all(example.text)
        answer, ms = _timed(lambda: self._backend.complete(abstracted))
        return BaselineRun(
            name=self.name,
            outbound_text=abstracted,
            answer=answer,
            routing_path="standard",
            latency_ms=ms,
        )
